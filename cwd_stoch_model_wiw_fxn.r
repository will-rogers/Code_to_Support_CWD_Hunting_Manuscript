#' CWD who-infects-who stochastic model 
#'
...
#'  conducts one run of the model. 2x2 matrix of 
...
params <- list(fawn.an.sur = 0.6, juv.an.sur = 0.8, ad.an.f.sur = 0.95,
               ad.an.m.sur = 0.9, fawn.repro = 0, juv.repro = 0.6, ad.repro = 1,
               hunt.mort.fawn = 0.01, hunt.mort.juv.f = 0.1, hunt.mort.juv.m = 0.1,
               hunt.mort.ad.f = 0.2, hunt.mort.ad.m = 0.000001, ini.fawn.prev = 0.02,
               ini.juv.prev = 0.03, ini.ad.f.prev = 0.04,  ini.ad.m.prev = 0.04,
               n.age.cats = 12,  p = 0.43, env.foi = 0,  beta.ff = 0.06,
               gamma.mm = 2, gamma.mf = 2, gamma.fm = 1,
               theta = 1, n0 = 1000, n.years = 10, rel.risk = 1.0,
...
cwd_stoch_model_wiw <- function(params) {
  ...
  for (t in 2:(n.years * 12)) {
    ...
    if (hunt.mort.ad.f >= 0.01){
      hunt.f.draw <- rbeta(n.age.cats - 2, hunt.f.b$alpha, hunt.f.b$beta, ncp = 0)
    }
    if (hunt.mort.ad.m >= 0.01){
      hunt.m.draw <- rbeta(n.age.cats - 2, hunt.m.b$alpha, hunt.m.b$beta, ncp = 0)
    }
    if (hunt.mort.ad.f < 0.01){
      hunt.f.draw <- rep(hunt.mort.ad.f, (n.age.cats - 2))
    }
    if (hunt.mort.ad.m < 0.01){
      hunt.m.draw <- rep(hunt.mort.ad.m, (n.age.cats - 2))
    }
    ...
    foi.f <- 1 - exp(- (beta.ff * sum(It.f[ ,t, ])/Nall^theta + 
                          beta.mf * sum(It.m[ ,t, ])/Nall^theta))
    foi.m <- 1 - exp(- (beta.mm * sum(It.m[ ,t, ])/Nall^theta + 
