...
require(stringr)
require(CWDsims)
df <- read.csv("R0.Scenarios.extreme.6.8.csv")
###### Example ######
...
params <- list(fawn.an.sur = 0.354, 
               ...
               n.years = 25, 
               rel.risk = 1.0)
outa <- cwd_wiw_track_R0(params = params)
outa$R0
outc$R0
plot_prev_age_end(outc$counts)
plot_prev_age_end(outa$counts)
plot_tots(outa$counts)

df$prev.rec <- NA
df$prev.rec[1] <- df$B.ff[1]
df$prev.rec[2] <- 3.075e-02
df$prev.rec[3] <- 6.89e-02


df$r.rec <- NA
df$r.rec[1] <- df$B.ff[1]
df$r.rec[2] <- 3.018e-02
df$r.rec[3] <- 7.355e-02
...
cwd_wiw_track_R0 <- function (params) {
  ...
  R0.ngm.approx <- R0_NGM(params)
  
  R0.prev.approx <- R0_from_prev(counts.long)
  
  R0.r.approx <- R0_from_r(counts.long, p)
  
  R0.point <- data.frame(value = c(R0.ngm.approx, R0.prev.approx),
